...
Tài liệu runtime/API backend tổng hợp nằm trong file này. Tài liệu kiến trúc và kiểm thử được công bố trên nhánh `codex/project-documentation`.
...
- Trước UAT/demo nên kiểm tra riêng frontend, email thật và các flow liên hệ nhiều service theo tài liệu trên nhánh `codex/project-documentation`.
...
Frontend gọi các API analytics qua các controller được khai báo trong module `analytics`.
...
- Dữ liệu job mẫu được tạo bằng Flyway seed; hệ thống không yêu cầu bộ dữ liệu crawl để khởi động.

Job được tạo qua Recruiter API hoặc Flyway seed. Vector hóa và matching được xử lý bởi service/scheduler của Backend.
