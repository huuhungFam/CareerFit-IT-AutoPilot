...
import com.careerfit.backend.cv.service.CvReviewAnalyzer;
...
public class CvManagementService {
    ...
    @Transactional
    public CvDtos.CvDetailResponse updateReview(UUID cvId, UUID userId, CvDtos.UpdateCvReviewRequest request) {
        CV cv = findAndAuthorize(cvId, userId);
        if (cv.getStatus() != CV.CvStatus.REVIEW_REQUIRED && cv.getStatus() != CV.CvStatus.DRAFT) {
            throw AppException.badRequest("CV can only be edited while review is required");
        }
        if (request.displayName() != null && !request.displayName().isBlank()) {
            cv.setDisplayName(request.displayName().trim());
        }
        Map<String, String> sections = request.sections() == null
                ? reviewSections(cv)
                : cleanSections(request.sections());
        cv.setReviewSectionsJson(write(sections));
        cv.setRawText(reviewAnalyzer.toRawText(sections));
        cv.setReviewIssuesJson(write(reviewAnalyzer.analyze(sections, List.of())));
        cv.setStatus(CV.CvStatus.REVIEW_REQUIRED);
        return toDetail(cvRepo.save(cv));
    }
    ...
    private Map<String, String> cleanSections(Map<String, String> input) {
    ...
    private String write(Object value) {
        try {
            ...
            throw AppException.badRequest("Could not save CV review content");
        }
    }
}
