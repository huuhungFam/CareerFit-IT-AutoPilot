...
public interface ApplicationRepository extends JpaRepository<Application, UUID> {
    ...
    @Query("""
        ...
        FROM Application a
        WHERE a.cv.id = :cvId AND a.job.recruiter.id = :recruiterId
        """)
    boolean existsByCvIdAndJobRecruiterId(@Param("cvId") UUID cvId, @Param("recruiterId") UUID recruiterId);
    ...
    List<Application> findByCandidateIdAndJobIdIn(UUID candidateId, List<UUID> jobIds);
    ...
    @Query("SELECT a FROM Application a WHERE a.candidate.id = :candidateId AND a.invitationWithdrawn = false ORDER BY a.appliedAt DESC")
    Page<Application> findByCandidateIdOrderByAppliedAtDesc(@Param("candidateId") UUID candidateId, Pageable pageable);
    ...
    @Query("""
        ...
        WHERE a.job.id = :jobId
          AND a.invitationWithdrawn = false
    ...
    @EntityGraph(attributePaths = {"candidate", "candidate.user", "cv", "matching", "job"})
    @Query("SELECT a FROM Application a WHERE a.job.id = :jobId AND a.invitationWithdrawn = false")
    List<Application> findAllByJobIdWithDetails(@Param("jobId") UUID jobId);

    @EntityGraph(attributePaths = {"candidate", "candidate.user", "cv", "matching", "job"})
    @Query("SELECT a FROM Application a WHERE a.job.id = :jobId AND a.invitationOrigin = true AND a.invitationWithdrawn = false ORDER BY a.updatedAt DESC")
    List<Application> findInvitationHistoryByJobId(@Param("jobId") UUID jobId);
