...
public interface MatchingRepository extends JpaRepository<Matching, UUID> {
    ...
    @Query("""
        ...
        FROM Matching m
        WHERE m.cv.id = :cvId AND m.job.recruiter.id = :recruiterId
        """)
    ...
    List<Matching> findByJobIdAndCvIdIn(UUID jobId, List<UUID> cvIds);

    @EntityGraph(attributePaths = {"job", "job.recruiter"})
    List<Matching> findByCvIdAndJobIdIn(UUID cvId, List<UUID> jobIds);
