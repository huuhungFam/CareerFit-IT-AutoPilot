...
public class QualityValidationService {
    ...
    public List<ValidationDtos.QualitySignal> validateCreateJob(JobDtos.CreateJobRequest req) {
        List<ValidationDtos.QualitySignal> signals = previewCreateJob(req);
        failOnErrors(signals);
        return signals;
    }

    public List<ValidationDtos.QualitySignal> previewCreateJob(JobDtos.CreateJobRequest req) {
        return previewJob(new JobDtos.JobQualityPreviewRequest(
                req.originalText(), req.requiredSkills(), req.seniorityLevel(), req.employmentType(),
                req.salaryMode(), req.salaryMin(), req.salaryMax(), req.salaryCurrency(), req.salaryType()));
    }

    public List<ValidationDtos.QualitySignal> previewJob(JobDtos.JobQualityPreviewRequest req) {
        List<ValidationDtos.QualitySignal> signals = new ArrayList<>();
        validateSalary(req.salaryMode(), req.salaryMin(), req.salaryMax(), signals);
        validateJobContent(
                req.originalText(), req.requiredSkills(), req.seniorityLevel(), req.employmentType(),
                req.salaryMax() != null ? req.salaryMax() : req.salaryMin(), req.salaryCurrency(), req.salaryType(), signals);
        return signals;
    }
