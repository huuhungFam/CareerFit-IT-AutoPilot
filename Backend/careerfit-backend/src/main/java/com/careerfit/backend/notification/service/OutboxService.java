...
public class OutboxService {
    ...
    /**
     * Queues one actionable CV-ready match email. The matching ID is the durable
     * dedupe key, so a CV scan can select several jobs without re-sending one JD.
     */
    @Transactional
    public boolean enqueueCvMatchNotification(UUID recipientId, UUID matchingId, UUID jobId,
                                              Instant scheduledAt, boolean demoMode) {
        UserAccount recipient = userAccountRepository.findByIdForUpdate(recipientId)
                .orElseThrow(() -> new IllegalArgumentException("Outbox recipient does not exist"));
        var decision = notificationPolicyGuard.evaluate(recipient, "MATCH_NOTIFICATION", matchingId.toString());
        if (!decision.allowed()) {
            notificationPolicyGuard.logSkipped(recipient, "MATCH_NOTIFICATION", matchingId.toString(), decision.reason());
            return false;
        }
        Instant effectiveScheduledAt = scheduledAt;
        if (demoMode) {
            Instant latest = outboxRepository.latestCvMatchNotificationSlot(recipientId);
            Instant first = Instant.now().plusSeconds(15);
            effectiveScheduledAt = latest == null
                    ? max(scheduledAt, first)
                    : max(scheduledAt, latest.plusSeconds(30));
        }
        return enqueueInternal(recipientId, "MATCH_NOTIFICATION", matchingId, jobId, effectiveScheduledAt);
    }
