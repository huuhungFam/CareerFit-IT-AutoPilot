...
public class MatchingService {
    ...
    private Matching upsertMatching(CV cv, Job job, boolean notifyHighMatch) {
        ...
        if (notifyHighMatch) enqueueEligibleMatch(matching);
        return matching;
    }
    ...
    private void notifyCandidateAfterScoring(CV cv, int activeJobCount, int scored) {
        ...
        // A fresh CV can score against hundreds of imported jobs. Only the five
        // strongest matches become mail notifications; every other matching stays
        // visible in the Candidate UI without creating an email flood.
        var policy = automationPolicyService.getOrCreate(user.getId());
        boolean demoMode = policy != null && policy.isDemoModeEnabled();
        matchingRepo.findTopMatchesByCvId(cv.getId(), PageRequest.of(0, 5))
                .forEach(match -> outboxService.enqueueCvMatchNotification(
                        user.getId(), match.getId(), match.getJob().getId(),
                        java.time.Instant.now(), demoMode));
    }
}
