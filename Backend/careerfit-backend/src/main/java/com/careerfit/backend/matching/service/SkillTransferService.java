...
/**
 * Versioned, explainable skill-transfer knowledge base used for Potential and
 * for structured required/optional skill coverage in the hybrid match score.
 */
@Service
public class SkillTransferService {
    ...
    /**
     * Produces the structured signals used by the primary matching score.
     * The assessment is deliberately unavailable when a recruiter did not
     * provide required skills, so legacy/imported text-only jobs retain the
     * deterministic TF-IDF score instead of receiving guessed requirements.
     */
    public StructuredFitAssessment assessStructuredFit(
            CV cv,
            Job job,
            Map<String, Double> cvVector
    ) {
        ensureLoaded();
        Set<String> required = parseStructuredSkills(job.getRequiredSkillsJson());
        if (required.isEmpty()) return StructuredFitAssessment.unavailable();

        Set<String> candidate = extractCandidateSkills(cv, cvVector);
        List<TargetCompatibility> requiredCompatibility = required.stream()
                .map(target -> bestStructuredCompatibility(candidate, target, cv.getRawText()))
                .toList();

        Set<String> optional = parseStructuredSkills(job.getNiceToHaveSkillsJson());
        List<TargetCompatibility> optionalCompatibility = optional.stream()
                .map(target -> bestStructuredCompatibility(candidate, target, cv.getRawText()))
                .toList();
        SeniorityAssessment seniority = assessSeniority(cv.getRawText(), job.getSeniorityLevel());

        List<String> matched = requiredCompatibility.stream()
                .filter(item -> item.score() >= 0.85)
                .map(TargetCompatibility::target)
                .toList();
        List<String> missing = requiredCompatibility.stream()
                .filter(item -> item.score() < MIN_TRANSFER_EDGE)
                .map(TargetCompatibility::target)
                .toList();

        return new StructuredFitAssessment(
                true,
                round(weightedCoverage(requiredCompatibility)),
                optional.isEmpty() ? 0.0 : round(weightedCoverage(optionalCompatibility)),
                !optional.isEmpty(),
                seniority.score(),
                seniority.severeGap(),
                matched,
    ...
    private Set<String> parseStructuredSkills(String json) {
        LinkedHashSet<String> skills = new LinkedHashSet<>();
        addJsonSkills(skills, json, true);
        return skills;
    }

    private double weightedCoverage(List<TargetCompatibility> compatibility) {
        double totalWeight = compatibility.stream()
                .mapToDouble(item -> skillWeight(item.target()))
                .sum();
    ...
    private TargetCompatibility bestStructuredCompatibility(
            Set<String> candidateSkills,
            String target,
            String cvText
    ) {
        TargetCompatibility known = bestCompatibility(candidateSkills, target);
        if (known.score() > 0.0) return known;

        // Recruiters can enter catalog extensions such as "TanStack Query".
        // Preserve exact phrase evidence even before that skill is promoted to
        // the versioned transfer model; do not infer transfers for unknown skills.
        if (containsWord(normalize(cvText), target)) {
            return new TargetCompatibility(target, target, 1.0);
        }
        return known;
    }
    ...
    private String extractSeniority(String text) {
        ...
        // CVs commonly state years of experience without an explicit level.
        // Support both English and Vietnamese wording before falling back to unknown.
        java.util.regex.Matcher years = java.util.regex.Pattern
                .compile("(?:^|\\s)(\\d{1,2})(?:\\+)?\\s*(?:years?|năm)(?:\\s|$)")
                .matcher(value);
        int maximum = -1;
        while (years.find()) maximum = Math.max(maximum, Integer.parseInt(years.group(1)));
        if (maximum >= 5) return "SENIOR";
        if (maximum >= 2) return "MID";
        if (maximum >= 0) return "JUNIOR";
        return null;
    }
    ...
    public record StructuredFitAssessment(
            boolean available,
            double requiredCoverage,
            double optionalCoverage,
            boolean hasOptionalSkills,
            double seniorityScore,
            boolean severeSeniorityGap,
            List<String> matchedRequiredSkills,
            List<String> missingRequiredSkills
    ) {
        static StructuredFitAssessment unavailable() {
            return new StructuredFitAssessment(false, 0.0, 0.0, false,
