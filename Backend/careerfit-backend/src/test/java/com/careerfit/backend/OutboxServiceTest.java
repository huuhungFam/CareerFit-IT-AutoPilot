...
class OutboxServiceTest {
    ...
    @Test
    void cvMatchNotificationIsQueuedPerMatchingAfterFifteenSecondsInDemoMode() {
        NotificationOutboxRepository repo = mock(NotificationOutboxRepository.class);
        when(repo.enqueueIdempotent(any(), any(), anyString(), anyString(), anyString(), any())).thenReturn(1);
        UserAccountRepository users = mock(UserAccountRepository.class);
        UUID recipient = UUID.randomUUID();
        when(users.findByIdForUpdate(recipient)).thenReturn(java.util.Optional.of(
                new UserAccount("candidate@example.test", "hash", UserAccount.Role.CANDIDATE, "Candidate")));
        when(repo.latestCvMatchNotificationSlot(recipient)).thenReturn(null);
        OutboxService service = new OutboxService(repo, users, allowedGuard());
        Instant now = Instant.now();

        boolean queued = service.enqueueCvMatchNotification(recipient, UUID.randomUUID(), UUID.randomUUID(), now, true);

        assertThat(queued).isTrue();
        ArgumentCaptor<Instant> scheduled = ArgumentCaptor.forClass(Instant.class);
        verify(repo).enqueueIdempotent(any(), eq(recipient), eq("MATCH_NOTIFICATION"), eq("MATCHING"), anyString(), scheduled.capture());
        assertThat(scheduled.getValue()).isAfterOrEqualTo(now.plusSeconds(15));
    }

    @Test
    void laterCvMatchNotificationUsesThirtySecondSpacingInDemoMode() {
        NotificationOutboxRepository repo = mock(NotificationOutboxRepository.class);
        when(repo.enqueueIdempotent(any(), any(), anyString(), anyString(), anyString(), any())).thenReturn(1);
        UserAccountRepository users = mock(UserAccountRepository.class);
        UUID recipient = UUID.randomUUID();
        when(users.findByIdForUpdate(recipient)).thenReturn(java.util.Optional.of(
                new UserAccount("candidate@example.test", "hash", UserAccount.Role.CANDIDATE, "Candidate")));
        Instant now = Instant.now();
        Instant previous = now.plusSeconds(20);
        when(repo.latestCvMatchNotificationSlot(recipient)).thenReturn(previous);
        OutboxService service = new OutboxService(repo, users, allowedGuard());

        service.enqueueCvMatchNotification(recipient, UUID.randomUUID(), UUID.randomUUID(), now, true);

        ArgumentCaptor<Instant> scheduled = ArgumentCaptor.forClass(Instant.class);
        verify(repo).enqueueIdempotent(any(), eq(recipient), eq("MATCH_NOTIFICATION"), eq("MATCHING"), anyString(), scheduled.capture());
        assertThat(scheduled.getValue()).isAfterOrEqualTo(previous.plusSeconds(30));
    }
