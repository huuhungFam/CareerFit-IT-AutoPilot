...
replacement = '''          <div className="jd-main-content">
            ...
            <p>{
              (() => {
                ...
                const m = text.match(/(?:M.? t.? C.?ng vi.?c|Trách nhiệm)(.*?)(?=Y.?u C.?u C.?ng Vi.?c|Yêu Cầu Công Việc|Quy.?n l.?i|$)/);
            ...
            <p>{
              (() => {
                const text = job.description || '';
                const m = text.match(/(?:Y.?u C.?u C.?ng Vi.?c|Yêu Cầu Công Việc)(.*?)(?=Quy.?n l.?i|$)/);
                return m ? m[1].trim() : '';
              })()
            }</p>
