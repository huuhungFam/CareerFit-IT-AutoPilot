...
test.describe('Phase 2 settings and catalog UI', () => {
  test('candidate controls Demo Mode and sees the effective timing', async ({ page }) => {
    ...
    const demoToggle = page.locator('label.setting-toggle', { hasText: 'Enable Demo Mode' }).getByRole('checkbox');
    ...
    await update;
    await expect(page.getByText(/Polling: 5s, First Delay: 12s, Subsequent: 30s/)).toBeVisible();
  });
