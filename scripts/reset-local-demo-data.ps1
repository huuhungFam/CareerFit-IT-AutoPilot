...
try {
    ...
    Assert-Scalar "SELECT version FROM flyway_schema_history WHERE success ORDER BY installed_rank DESC LIMIT 1;" '40' 'latest Flyway migration'
    ...
    # Docker volume timestamps are second-granular on some Desktop setups, so
    # comparing them with the script start time can reject freshly recreated
    # volumes created within that same second.  The validated deletion above
    # and the Compose labels here are the reliable reset proof.
    Write-Host 'PASS: recreated volumes have the expected Compose ownership'
